...
control 'azure-service-environment' do
  ...
  describe azure_service_environment(rg_name: 'sandbox-core-infra-dev', name: 'app-compute-sandboxtestsupport') do
    its('location') { should eq 'UK South' }
    its('name') { should eq 'app-compute-sandboxtestsupport' }
    its('vnet_name') { should eq 'sandbox-core-infra-vnet-dev' }
    its('vnet_resource_group_name') { should eq 'sandbox-core-infra-dev' }
    its('vnet_subnet_name') { should eq 'sandbox-core-infra-subnet-2-dev' }
