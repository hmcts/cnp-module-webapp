...
| `https_only`               | String  | No       | `"false"`   | Configures a web site to accept only https requests. Issues redirect for http requests. Accepts string values `"true"` or `"false"`.                                                                                                                                         |
...
| `java_version`             | String  | No       | `1.8`       | The Java version to run on. See the Azure portal for supported values, e.g. `1.8`, `11`, `17`, `21`.                                                                                                                                                                         |
| `tomcat_version`           | String  | No       | `9.0`       | The version of Tomcat the Java app should use. See the Azure portal for available values, e.g. `9.0`, `10.0`.                                                                                                                                                                |
...
```python
assertIn('resource "azurerm_windows_web_app" "app_service_site"', main_tf)
```
...
All infrastructure provisioning is done using the Terraform native `azurerm` provider (`~> 4.62`). You can find the documentation for this at the following link:-

[Terraform azurerm](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs) <br />

Resources are created using native provider resources (`azurerm_windows_web_app`, `azurerm_service_plan`, `azurerm_traffic_manager_profile`, etc.). No ARM templates are used.
